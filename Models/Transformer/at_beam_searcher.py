...
class ATPredictor:
    ...
    def __init__(self, args):
        ...
        self.depth = 3 #Depth for beam search
    ...
    def overwrite_model_args(self):
        ...
        self.model_args.src = os.path.join(self.processed_data_path, "test.txt")
        ...
        with open(self.output_file, "wb") as file:
            ...
            for i, (src_shard, tgt_shard) in tqdm(enumerate(shard_pairs)):
                ...
                while len(terminated_node) < k:
                    ...
                    try:
                        # smi = 'CC1(OC(=O)OCI)CCCCC1.CCn1nc(C2=C(C(=O)O[Na])N3C(=O)[C@H]([C@@H](C)O)[C@H]3C2)cc1C'
                        # src_smiles = [' '.join(smi)]
                        # print('#' * 80)
                        # print(f'src_smiles: {src_smiles}')
                        # print('#' * 80)
    ...
    def score(self):
        ...
        with open(self.output_file, 'rb') as file:
            ...
            for result in tqdm(p.imap(pickle2kv, (prediction_row for prediction_row in prediction_reader))):
                k, v = result
                predictions[k] = v
        ...
        with open(self.test_unseen_path, "r") as test_csv:
            ...
            accuracies = p.imap(match_results,
                                ((test_row, n_best, k) for k, test_row in enumerate(test_reader)))
